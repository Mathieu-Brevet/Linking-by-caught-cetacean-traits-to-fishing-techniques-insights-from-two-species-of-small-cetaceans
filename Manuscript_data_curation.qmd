...
# we only retrieved here fishing activity information that will use latter on for complement our original dataset
...
#minimal requirement for being able to run the previous section: 16GB of RAM
...
The purpose of this code is to format the data so that it can be analysed later. Three datasets were used: the fisheries observer data (OBSMER data) and the French data on reported fishing activities (SACROIS data, which also includes the skipper declaration of by-catches). We prepared data to retain only variables used for analysis, time and areas of interest, while removing potentially error-prone data.
...
# there is no individual traits measurements for declaration of multiple capture, we then discarded such declarations from analyses:
...
# We discarded all data with individuals weighing less than 3 kg or more than 300 kg (range of values outside the weight ranges existing in natural conditions)
...
# We discarded all data with individuals weighing more than 150 kg (range of values outside the weight ranges existing in natural conditions)
...
# Selection of areas of interest and retrieval of ICES division associated with location points of strandings:
...
# Removal of data without any phenotypic measure:
...
OBSMER_BYC=merge(OBSMER_BYC, marees, by="ID_MAREE", all.x = T, all.y=F)
...
OBSMER_BYC=rbind(OBSMER_BYC[NOMBRE.y==1 | is.na(NOMBRE.y)], OBSMER_BYC[NOMBRE.y>1][rep(sequence(nrow(OBSMER_BYC[NOMBRE.y>1])), OBSMER_BYC[NOMBRE.y>1, NOMBRE.y]),]) # for the number indicated in measurements, number superior to one correspond to dolphin having the same measurements (we therefore repeated these lines)
    ...
    if (u[i]==1 | (u[i]>1 & cat[i]==1)) {
      ...
      for (j in 1:length(OBSMER_BYC[ID_CAPTURE==unique(OBSMER_BYC$ID_CAPTURE)[i], ID_CAPTURE])) {
        OBSMER_BYC[ID_CAPTURE==unique(OBSMER_BYC$ID_CAPTURE)[i],][j,]$ID_IND=
          paste(OBSMER_BYC[ID_CAPTURE==unique(OBSMER_BYC$ID_CAPTURE)[i],]$ID_CAPTURE[1],j, sep="_")
      } # individuals measured in groups, with the same capture ID, and with multiple measures: considering the different measures as different individuals as we are not able to reliably assigned the different measurements to each individual
    }
}
...
OBSMER_BYC[ESPECE=="Phocoena phocoena" & POIDS_REF.x=="150.0", POIDS_REF.x:=""] # discarding all data with individuals weighing more than 150 kg (range of values outside the weight ranges existing in natural conditions)
...
# removal of 4 measures of circonference completely aberrant (<30cm for weight >100kg, or >180cm...)
...
# trip IDs from declaration and observation datasets are not attributed the same way
...
} # retrieving the most probable declaration fishing trip ID (on the basis of landing harbour and fishing trip timing) for each vessels of potential overlap between declaration and observation data
...
OBSMER_BYC_Short_note = merge(OBSMER_BYC_Short_note[, TARGETED_TAXA_ID := str_split_i(ESPECE_CIBLE, " - ", 1)], Table_correspondence[, 1:2], by.x = "TARGETED_TAXA_ID", by.y = "ESPF_COD", all.x = T, all.y = F) # retrieving ISSCAAP id associated with targeted taxa
