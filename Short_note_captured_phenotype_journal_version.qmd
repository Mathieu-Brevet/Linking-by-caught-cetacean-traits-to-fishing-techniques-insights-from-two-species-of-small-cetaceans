...
library(stringr)#gestion chaines de caractères
library(car)#anova ou graph d'analyse
library(RVAideMemoire)#variance
library(corrplot)# graphe correlation
library(ggeffects) #effet prédit
library(ggplot2) #graphs
library(ggtext) #customization of ggplot legends
library(ggpubr) #custom graph (comparison bars)

library(multcomp) #multiple comparison of contrasts

library(data.table) #large dataset gestion 

library(lme4) #mixed model
library(blme) #bayesian mixed model (handling singular fit)
library(DHARMa) #model diagnostic (mixed models/non linear models)
library(MuMIn) #r-squared for glmm (Nakagawa method)
...
library(texreg) #export table in Latex format
library(kableExtra)



library(MASS) #used to generate multivariate normal distribution
...
Strandings <- fread("~/ATER PAU 2024/Papier postdoc sensibilité bycatch/Strandings_journal_version.csv")

# Declaration data:

Declaration_Bycatch = fread("~/ATER PAU 2024/Papier postdoc sensibilité bycatch/Declaration_Bycatch_journal_version.csv")

# Observation data (OBSMER programme):

OBSMER_Bycatch <- fread("~/ATER PAU 2024/Papier postdoc sensibilité bycatch/OBSMER_Bycatch_journal_version.csv")
...
# Merging data in common between declaration and observation datasets:
...
# Merging data in common between declaration and observation datasets, by only considering explanatory variables:
...
# Decl_Obs_Merged_Bycatch = all_merged[, c("ORIGIN", "ESP_LIB_FAO", "SECT_COD_SACROIS_NIV3", "SECT_COD_SACROIS_NIV3b", "TRIMESTRE", "AN", "YEAR_DCO", "YEAR_PHO", "ENGIN2", "ENGIN4", "MAILLAGE", "TARGET_TAXON", "TARGET_TAXON3", "TARGET_TAXON4", "poids_kg")]
...
apply(Declaration_Bycatch[SPECIES %in% c("Delphinus delphis", "Phocoena phocoena"),], 2, function(x) table(is.na(x))) # in particular, targeted taxa were reported in ~68% of cases (very few missing values otherwise)
...
# Maps of corrected abundance:



observation_effort <- readRDS("~/ATER PAU 2024/Papier postdoc sensibilité bycatch/observation_effort.rds")
...
allgeo <- readRDS("~/ATER PAU 2024/Papier postdoc sensibilité bycatch/allgeo_Journal_version.rds") # map of ICES division of interest
...
ggsave("Figure_S2.pdf", device = "pdf", units = "in", width = 14, height = 8.5)
...
ggsave("Figure_S3A.pdf", device = "pdf", units = "in", width = 14, height = 8.5)
...
ggsave("Figure_S3B.pdf", device = "pdf", units = "in", width = 14, height = 8.5)
...
ggsave("Figure_S3C.pdf", device = "pdf", units = "in", width = 14, height = 8.5)
...
ggsave("Figure_S5.pdf", device = "pdf", units = "in", width = 14, height = 8.5)
...
# Creation of datasets per species:
...
# key population values reported in the literature for each of the species studied (for both sexes: body length at maturity, asymptotic adult body length)
...
# some years appears quite rarely and the number of categories associated with the variable is high
...
# only model on size are performed because this measure is systematically performed for stranded individuals (contrary to weight) and more reliable in this specific case
...
### Table combining pairwise contrast comparison test for each significant variable either in models studying sex distribution (S3) or size distribution (S4):
...
ggplot(dcast(na.omit(rbind(OBSMER_Bycatch[!is.na(SEX) & SPECIES %in% c("Delphinus delphis", "Phocoena phocoena"), .(SEX, SPECIES, "Captures")], Strandings[!is.na(SEX), .(SEX, SPECIES, "Strandings")])), SEX + SPECIES + V3 ~ ., fun.aggregate = length), aes( x =V3, y = .)) +
  geom_bar(aes(fill = SEX), stat = "identity", position = "dodge") + 
...
ggplot(rbind(Decl_Obs_Merged_Bycatch[SPECIES %in% c("Delphinus delphis", "Phocoena phocoena"), .(BODY_MASS, SPECIES, "Captures")], Strandings[!is.na(SEX), .(BODY_MASS, SPECIES, paste0("Strandings,\n", SEX))]), aes( x = V3, y = BODY_MASS)) +
  geom_violin() + geom_boxplot(width = 0.1) +  
  ggtitle("Body mass of captured/stranded individuals") + 
...
ggplot(rbind(OBSMER_Bycatch[, .(paste0("*", SPECIES, "*", ", ", SEX, "<br>Captures"), BODY_LENGTH)], Strandings[, .(paste0("*", SPECIES, "*", ", ", SEX, "<br>Strandings"), BODY_LENGTH)]), aes( x = V1, y = BODY_LENGTH)) + 
  geom_violin(width = 1) + geom_boxplot(width = 0.1) + geom_point(data=biblio, mapping=aes(x = param, y = VALEUR, shape=ref), size=2) + 
  ggtitle("Body length of captured/stranded individuals\n(compared with literature life-history traits)") +
  labs(x = "Species, sex, data sources and literature estimates", y = "Body length (cm)", shape= "Bibliographic\nreferences") +  
  theme(text = element_text(size = 15))  +  theme(axis.text.x = element_text(angle = 60, hjust = 1))  + 
  scale_x_discrete(limits = c("*Delphinus delphis*, Female<br>Strandings","*Delphinus delphis*, Female<br>Captures","Sexual maturity<br>(females, *Delphinus delphis*)","Asymptotic length<br>(females, *Delphinus delphis*)","*Delphinus delphis*, Male<br>Strandings","*Delphinus delphis*, Male<br>Captures","Sexual maturity<br>(males, *Delphinus delphis*)","Asymptotic length<br>(males, *Delphinus delphis*)", "*Phocoena phocoena*, Female<br>Strandings","*Phocoena phocoena*, Female<br>Captures","Sexual maturity<br>(females, *Phocoena phocoena*)","Asymptotic length<br>(females, *Phocoena phocoena*)","*Phocoena phocoena*, Male<br>Strandings","*Phocoena phocoena*, Male<br>Captures","Sexual maturity<br>(males, *Phocoena phocoena*)","Asymptotic length<br>(males, *Phocoena phocoena*)")) +
    theme(axis.text = element_markdown())
...
ggsave("Figure_S11.pdf", device = "pdf", units = "in", width = 14, height = 8.5)
...
table(OBSMER_Bycatch[!is.na(SEX) & SPECIES == "Delphinus delphis"]$SEX, OBSMER_Bycatch[!is.na(SEX) & SPECIES == "Delphinus delphis"]$ICES_DIV_grouped) # more female capture north of the bay of Biscay (not significant)
...
# Prior to spatio-temporal test we checked whether body mass variation depended on the data sources:
...
# as their is some sea state categories with too little data (highest and lowest sea state measured) we grouped the most extreme conditions together for analyses:
...
ggsave("Figure_S16A.pdf", device = "pdf", units = "in", width = 14, height = 8.5)
...
ggplot(OBSMER_Bycatch[!is.na(BODY_LENGTH) & SPECIES == "Delphinus delphis",], aes(BODY_LENGTH, SEA_STATE_grouped)) + geom_violin() + geom_boxplot(width = 0.1) +  stat_compare_means(label = "p.format", comparisons = list(c("2 - belle, vagues de 0.1 à 0.5 mètres", "Calme ou ridée"), c("3 - peu agitée, vagues de 0.5 à 1.25 mètres", "Calme ou ridée"), c("4 - agitée, vagues de 1.25 à 2.5 mètres", "2 - belle, vagues de 0.1 à 0.5 mètres"), c("Forte à grosse", "Calme ou ridée")), size=9) + ggtitle("Common dolphin - BL model - OBSMER dataset") +  labs(y = "Sea conditions (Douglas scale)", x = "Captured individuals' body length (cm)") +  theme(text = element_text(size = 30)) + scale_y_discrete(labels=c("Calm (0-1)", "Smooth (2)", "Slight (3)", "Moderate (4)", "Rough (>5)"))

 ggsave("Figure_S16B.pdf", device = "pdf", units = "in", width = 14, height = 8.5)
...
# Susceptibility to be captured


We then explored the susceptibility of individuals to be captured as a function of fishing technique. These analyses initially considered only the risk of being captured (see first graphical outputs, this part was scanned quickly as it had already been thoroughly studied) and how this susceptibility varies with individual traits (i.e. phenotypic vulnerabilities to fishing technique, see subsequent regression models).
...
ggsave("Figure_S7A.pdf", device = "pdf", units = "in", width = 14, height = 8.5)
...
ggsave("Figure_S7B.pdf", device = "pdf", units = "in", width = 14, height = 8.5)
...
ggplot(Decl_Obs_Merged_Bycatch_explanatory[!is.na(TARGETED_TAXA) & SPECIES == "Delphinus delphis"], aes(x = forcats::fct_infreq(TARGETED_TAXA), fill = forcats::fct_infreq(TARGETED_ISSCAAP))) +
  ...
  scale_fill_discrete(labels = c("31 - Pleuronectiformes (N=105)", "32 - Gadiformes (N=91)", "33 - Miscellaneous coastal fishes (N=68)", "34 - Miscellaneous demersal fishes  (N=17)",  "36 - Tunas, bonitos (N=17)", "37 - Miscellaneous pelagic fishes (N=12)", "57 - Squids, cuttlefishes, octopuses (N=5)", "35 - Herrings, sardines, anchovies  (N=3)", "43 - Lobsters, spiny-rock lobsters (N=3)",  "38 - Sharks, rays (N=2)"))
...
ggplot(Decl_Obs_Merged_Bycatch_explanatory[!is.na(TARGETED_TAXA) & SPECIES == "Phocoena phocoena"], aes(x = forcats::fct_infreq(TARGETED_TAXA), fill = forcats::fct_infreq(TARGETED_ISSCAAP))) +
  ...
  theme(text = element_text(size = 20))   +    theme(axis.text.x = element_text(angle = 65, hjust = 1)) +
  scale_fill_discrete(labels = c("31 - Pleuronectiformes (N=44)", "34 - Miscellaneous demersal fishes  (N=32)", "32 - Gadiformes (N=12)", "33 - Miscellaneous coastal fishes (N=3)",  "38 - Sharks, rays (N=2)", "37 - Miscellaneous pelagic fishes (N=1)", "43 - Lobsters, spiny-rock lobsters (N=1)", "57 - Squids, cuttlefishes, octopuses (N=1)"))
...
We then studied how traits of by-caught individuals varied with fishing techniques used so as to identify potential phenotypic vulnerabilities. Three phenotypic traits were studied: the by-caught individuals' body mass (using both declaration and observation data), body length and sex (using observation data only). 
...
# We began to remove too rare categories (N<6) from regression analyses (while having removed all missing data with missing values for at least one of the considered explanatory variables):
...
# in this specific case there is rare categories in both categorical variables: removing rare categories in targeted ISSCAAP groups categories lead to the presence of too rare categories in used fishing gears. We therefore cannot use both variables in the model. In this particular case we chose to favor the use of used fishing gear in explanatory variables.
...
# some variable present too many missing values to be included in analyses (as gear speed only available for trawls)
...
# gear speed, dimension and depth were discarded from analyses due to too many missing values and/or because they were too well described by other variables (in this latter case including these varaibles in regression models resulted in too high VIF values)
...
table(OBSMER_Bycatch[SPECIES == "Phocoena phocoena" & !is.na(BODY_LENGTH) & !is.na(TARGETED_ISSCAAP) & !is.na(MESH_SIZE) & !is.na(FISHING_GEAR_grouped) & TARGETED_ISSCAAP %in% c("31 - Pleuronectiformes", "34 - Miscellaneous demersal fishes")]$FISHING_GEAR_grouped)
# in this specific case there is rare categories in both categorical variables: removing rare categories in targeted ISSCAAP groups categories lead to the presence of too rare categories in used fishing gears. We therefore cannot use both variables in the model. In this particular case we chose to favor the use of used fishing gear in explanatory variables.
...
ggsave("Figure_S14C.pdf", device = "pdf", units = "in", width = 14, height = 8.5)
...
ggsave("Figure_S14D.pdf", device = "pdf", units = "in", width = 14, height = 8.5)
...
ggsave("Figure_S14E.pdf", device = "pdf", units = "in", width = 14, height = 8.5)
...
############ Modifier legende axe abscisse pour simplifier interpretation ?????



#reordering variables for plots:

# OBSMER_size_del$ISSCAAP=factor(OBSMER_size_del$ISSCAAP, levels = c("31","38","32","36","37","33","34"), labels=c("Pleuronectiformes\n(mainly soles)", "Sharks, rays", "Gadiformes\n(mainly hake and gadoids)", "Tunas, bonitos", "Miscellaneous pelagic fishes\n(mainly mackerells)", "Miscellaneous coastal fishes\n(mainly sea bass)", "Miscellaneous demersal fishes\n(mainly monkfishes or St Pierre)")) # 31:poisson plats, 32:merlus et gadidés, 33: mainly bass, 34: démerseaux (baudroies, saint pierre), 36: thon, bonite, 37: maqueraux, chinchard, 38: requins, raies 
...
### Global table (Table S6) of vulnerabilities regression models (ANOVA, type II):
...
### Table (S7) combining pairwise contrast comparison test for each significant variable in vulnerabilities regression models:
...
setDT(TableS7)
TableS7[, lapply(.SD, function(x) ifelse(str_detect(x, "\n"), str_replace_all(x, "\n", " "), x))]
setDF(TableS7)
